...
pub(crate) fn is_omf<'data, R: crate::ReadRef<'data>>(data: R, offset: u64) -> bool {
    ...
    // Read the full record to validate it and check the name length
    ...
    // We intentionally do NOT check the checksum here — some Borland/Watcom
    // tools write non-zero but incorrect checksums, and rejecting such files
    // during detection would cause a spurious "Unknown file magic" error.
    if length > 1 {
