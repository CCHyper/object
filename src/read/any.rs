...
macro_rules! map_inner_option {
    ($inner:expr, $from:ident, $to:ident, | $var:ident | $body:expr) => {
        match $inner {
            ...
            #[cfg(feature = "omf")]
            $from::Omf(ref $var) => $body.map($to::Omf),
        }
    };
}
...
macro_rules! map_inner_option_mut {
    ($inner:expr, $from:ident, $to:ident, | $var:ident | $body:expr) => {
        match $inner {
            ...
            #[cfg(feature = "omf")]
            $from::Omf(ref mut $var) => $body.map($to::Omf),
        }
    };
}
...
macro_rules! next_inner {
    ($inner:expr, $from:ident, $to:ident) => {
        match $inner {
            ...
            #[cfg(feature = "omf")]
            $from::Omf(ref mut iter) => iter.next().map($to::Omf),
        }
    };
}
...
enum SegmentInternal<'data, 'file, R: ReadRef<'data>> {
    ...
    #[cfg(feature = "omf")]
    Omf(omf::OmfSegmentRef<'data, 'file, R>),
}
    ...
    R: ReadRef<'data>,
{
    ...
    #[cfg(feature = "omf")]
    Omf((omf::OmfSymbol<'data>, PhantomData<R>)),
}
