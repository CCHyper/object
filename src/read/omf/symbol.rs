...
pub struct OmfSymbol<'data> {
    ...
    /// - COMDEF/LCOMDEF: communal variable byte count (from the data-type field).
    /// - PUBDEF/LPUBDEF: inferred from adjacent symbols within the segment.
    /// - All others: 0.
    pub communal_size: u32,
    /// Type index (usually 0)
    pub type_index: u16,
    /// Pre-computed symbol kind
    pub kind: SymbolKind,
    /// True for weak externals declared via COMENT 0xA7 (WKEXT) or 0xA8 (LZEXT).
    pub is_weak: bool,
    /// COMDEF data type: 0x62 = NEAR (`virtual(_TEXT)`, code),
    /// 0x61 = FAR (`virtual(_DATA)`, data), 0x01..0x5F = scalar data.
    /// Zero for non-communal symbols.
    pub communal_data_type: u8,
}
...
pub enum OmfSymbolClass {
    ...
    ComdatExternal,
    /// Alias symbol (ALIAS record: alternate name for another symbol)
    Alias,
    /// Synthetic section symbol (one per segment, for relocation targets)
    Section,
}
...
impl<'data> ObjectSymbol<'data> for OmfSymbol<'data> {
    ...
    fn address(&self) -> u64 {
        // Communal symbols have no location; they're an unallocated size declaration.
        if self.is_common() {
            return 0;
        }
